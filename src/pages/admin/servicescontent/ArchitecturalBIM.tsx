...
export default function EditArchitecturalBIM() {
  ...
  const header = { head: "Live Editor: Architectural BIM Services" };
  ...
  const handleSave = async () => {
    ...
    try {
      await saveToGithub(
        "src/data/architecturalBIM.json",
        JSON.stringify(data, null, 2),
        githubToken,
        "Update Architectural BIM page content via Admin CMS"
      );
        ...
        <div className="flex flex-row h-full overflow-hidden">
          ...
          <div className="w-[550px] bg-white border-r flex flex-col h-full overflow-y-auto animate-fade-in">
            ...
            <div className="p-4 space-y-6">
              ...
              {/* Offerings Section */}
              <div className="pb-4 p-2 rounded" ref={offeringsRef}>
                <h3 className="text-lg font-semibold mb-2">Our BIM Offerings</h3>
                {data.offerings.map((offering, idx) => (
                  <div key={idx} className="flex flex-row items-center mb-2 gap-2">
                    <input
                      ...
                      value={offering}
                      onChange={(e) => setData(prev => {
                        const newOfferings = [...prev.offerings];
                        newOfferings[idx] = e.target.value;
                ...
                <button 
                  onClick={() => setData(prev => ({ ...prev, offerings: [...prev.offerings, "New Offering Item"] }))}
                  className="mt-2 w-full py-1.5 border border-dashed border-[#6abd45] text-[#6abd45] bg-white hover:bg-green-50 rounded-sm text-xs font-bold uppercase transition-all shadow-sm"
                >
                  + Add Offering Item
                </button>
